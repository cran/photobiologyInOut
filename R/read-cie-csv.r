...
#'  
#' @param file.name character string Name of CSV file.
#' @param label character string. If \code{NULL} metadata read from the
...
#'
#' @return Depending on the contents of the file, a \code{source_mspct} or
...
read_CIE_csv <- function(file.name,
  ...
  if (!file.exists(metadata.file.name)) {
    stop("Metadata file: '", metadata.file.name, "' not found.")
  }
  ...
  if (grepl("^CIE_illum|^CIE_std_illum", base.name)) {
    ...
    f <- "identity"
    target <- NA_real_
  } else if (grepl("^CIE_cc|^CIE_xyz", base.name)) {
    member.class <- "chroma_spct"
    spct.data.var <- c("x", "y", "z")
    f <- "identity"
    target <- NA_real_
  }
  ...
  if (member.class == "chroma_spct") {
    z <- cie.tb
    colnames(z) <- c("w.length", spct.data.var)
    z <- photobiology::as.chroma_spct(z)
    z <- photobiology::chroma_mspct(z)
  } else {
    z <- 
