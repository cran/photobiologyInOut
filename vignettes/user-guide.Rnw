...
Collections of spectra have a \texttt{spct.byrow} attribute but it is discarded as it describes dimensions would require spectral data to be stored in a three dimensional array and cannot be mapped to a matrix. The argument \code{byrow} in the conversion function has the same meaning as in the \texttt{matrix} constructor function.
...
Can export to "hyperSpec" objects only collections of spectra where all members have identical \texttt{w.length} vectors, as objects of class \texttt{hyperSpec} store a single vector of wavelengths for the whole collection of spectra.
